...
DataONE integrates into Whole Tale in three areas.
  1. Registering Data
      ...
      This Involves communicating with DataONE to locate data packages
      and requires backend support to mimic nested package structures
  2. Publishing Tales
      Whole Tale supports publishing Tales to DataONE.
      This Involves taking a set of girder files and creating a DataONE compliant package.
      It also involves authenticating a user with DataONE from Whole Tale
  3. Importing Tales
      Whole Tale supports importing tales from DataONE.
      This involves a UI addition to DataONE's UI package landing page
      along with adding support in Whole Tale for recognising Tales.

The level of integration varies between areas-ranging from user authentication
to simple link buttons.

Data Registration
-----------------
Registering data involves two steps: Searching for the package, and the actual
file registration.

The user should be allowed to search for a data package by specifying the package
in a number of formats, such as the doi or package landing page URL. The SOLR DataONE endpoint is used to
...
in Whole Tale is created. An EML_ document describing the package, and a 
...
.. _Data Registration: ../data-registration/index.rst
.. _Tale Publishing: ../tale-publishin/index.rst
.. _Tale Importing: ../tale-importing/index.rst
